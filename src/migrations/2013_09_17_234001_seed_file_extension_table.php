...
class SeedFileExtensionTable extends Migration {
	...
	public function up()
	{
		...
		{
			$modelId = DB::table('object_type')->where('code', 'file_extension')->pluck('id');
			...
			DB::table('object_field')->insertGetId(
					[
						'id' => DB::table('object_sequence')->insertGetId(['id' => null, 'class_model' => '\Telenok\Object\Field']),
						'title' => json_encode(['ru' => "Расширение", 'en' => "Extension"], JSON_UNESCAPED_UNICODE),
						'title_list' => json_encode(['ru' => "Расширение", 'en' => "Extension"], JSON_UNESCAPED_UNICODE),
						'key' => 'string',
						'code' => 'extension',
						'active' => 1,
						'field_object_type' => $modelId,
