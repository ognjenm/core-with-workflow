...
class SeedWidgetGroupTable extends Migration {
	...
	public function up()
	{
		...
		{
			$modelId = DB::table('object_type')->where('code', 'widget_group')->pluck('id');
			...
			DB::table('object_field')->insert(
					[
						'id' => DB::table('object_sequence')->insertGetId(['id' => null, 'class_model' => '\Telenok\Core\Model\Object\Field']),
						'title' => json_encode(['en' => 'Controller class', 'ru' => 'Класс контроллера'], JSON_UNESCAPED_UNICODE),
						'title_list' => json_encode(['en' => 'Controller class', 'ru' => 'Класс контроллера'], JSON_UNESCAPED_UNICODE),
						'key' => 'string',
